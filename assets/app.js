...
function apply() {
  ...
  if (state.me) {
    /* 140 installations du recensement n'ont pas de coordonnees. La haversine
       leur rendrait NaN, qui n'est ni plus grand ni plus petit que quoi que ce
       soit : le tri les aurait laissees ou le hasard voulait, et « _d != null »
       plus bas aurait affiche « NaN km ». Infinity les met en fin de liste, les
       exclut du filtre « autour de moi », et se laisse tester. */
    for (const t of out) {
      t._d = (t.lat == null || t.lon == null)
        ? Infinity
        : distance(state.me.lat, state.me.lon, t.lat, t.lon);
    }
    ...
    <li>
      ...
      <div class="card-main">
        <div class="card-top">
          ...
          ${Number.isFinite(t._d) ? `<span class="dist">${fmtDist(t._d)}</span>` : ''}
        </div>
...
function renderMap() {
  ...
  /* Une installation sans coordonnees n'a pas de place sur la carte, et
     L.marker([null, null]) leve « Invalid LatLng » — ce qui emporterait tout
     l'affichage, pas seulement ce point. Elle reste dans la liste. */
  const markers = state.shown.filter(t => t.lat != null && t.lon != null).map(t =>
...
function openSheet(id) {
  ...
  /* Sans coordonnees, `destination` prend l'adresse postale en clair : Google
     Maps l'accepte aussi bien qu'un couple de nombres, la ou « null,null »
     aurait ouvert un itineraire vers nulle part. Meme regle que la fiche
     statique, voir page_site() dans build_site.py. */
  const situe = t.lat != null && t.lon != null;
  const gmaps = 'https://www.google.com/maps/dir/?api=1&destination=' + (situe
    ? `${t.lat},${t.lon}`
    : encodeURIComponent([t.nom, t.adresse, t.cp, t.ville].filter(Boolean).join(', ')));
  ...
  $('#sheet-body').innerHTML = `
    ...
    <div class="actions">
      ...
      ${situe ? `<button class="btn" type="button" id="btn-minimap">${esc(U.voir_carte)}</button>` : ''}
